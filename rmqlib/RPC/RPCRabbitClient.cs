    ...
    where TResponse : IMessage<TResponse>, new()
{
    ...
    protected ConcurrentDictionary<string, ConcurrentBag<(int index, string transientStatus, byte[] payload)>>
        _responses =
            new();

    public RpcClient(string? hostname, string exchange, string user) : base(hostname, exchange, user)
    {
        _replyQueueName = QueueName + Guid.NewGuid().ToString() + ".reply";
        ...
        return Task.Run(() =>
        {
            ...
            _consumer.Received += (model, ea) =>
            {
                ...
                try
                {
                    ...
                    {
                        var cid = idparts[0];
                        var partIndex = double.TryParse(idparts[1], out var d) ? (int)d : -1;
